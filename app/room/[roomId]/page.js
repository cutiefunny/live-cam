...
import WebRTCManager, { initializePeer, destroyPeer } from '@/hooks/useWebRTC';
...
export default function Room() {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      // 2. Peer 객체 초기화
      initializePeer(user, iceServers, setPeers, showToast);
      ...
      WebRTCManager.setLocalStream(streamRef);
  ...
  useEffect(() => {
    if (myStream && otherUser && !peers[otherUser.uid] && !isCreator && user && user.uid > otherUser.uid) {
        WebRTCManager.callPeer(otherUser.uid);
    }
  }, [myStream, otherUser, peers, isCreator, user]);

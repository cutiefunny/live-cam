...
import styles from '@/app/Home.module.css'; // ✨ [수정] 경로 변경
...
function RatingTrigger() {
  ...
  useEffect(() => {
    ...
    if (callEnded === 'true' && creatorId && creatorName) {
      ...
      // ✨ [수정] 리디렉션 경로를 /creator로 변경
      router.replace('/creator', { shallow: true });
    }
...
// ✨ [수정] 컴포넌트 이름 변경 Home -> CreatorPage
export default function CreatorPage() {
  // ✨ [수정] signOut은 ProfileModal에서 필요하므로 useAuth 유지
  ...
  // ✨ [제거] if (!user) { ... } 로그인 블록 제거

  // ✨ [추가] user 객체가 없을 경우 (예: 로그아웃 시) 리디렉션 전까지 로딩 표시
  if (!user) {
    return <div className={styles.main}><div>Loading...</div></div>;
  }
